...
function calc_drone_min_energy(route) {

  const energyDeficit = 0;
  const balance = 0;
  const routeLenght = route.length;

  for (let i = 1; i < routeLenght; i++) {
    balance += route[i - 1][2] - route[i][2];
    if (balance < energyDeficit) {
      energyDeficit = balance;
    }
  }
  return Math.abs(energyDeficit);
}
